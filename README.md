...
- Hosted endpoint: `https://mcp.usecelina.xyz/api/mcp` — hosted read/prepare profile (currently **45 tools** via `HOSTED_TOOL_COUNT` in `src/data/tools.generated.ts`; no `estimate_*` or server-key writes). Public read-only — no API key; see [celina-mcp-host SECURITY.md](../celina-mcp-host/SECURITY.md).
