...
- Hosted endpoint: `https://mcp.usecelina.xyz/api/mcp` — remote read/prepare profile (currently **45 tools** via `HOSTED_TOOL_COUNT` in `src/data/tools.generated.ts`; no `estimate_*` or server-key writes). Public read-only — no API key; see [celina-mcp-remote SECURITY.md](../celina-mcp-remote/SECURITY.md).
...
src/
  ...
  data/tools.ts     # Merged tool docs — counts from tools.generated.ts (run npm run sync-tools)
